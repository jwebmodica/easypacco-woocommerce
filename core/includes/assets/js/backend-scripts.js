...
function showLoadingOverlay() {
    if (jQuery('#easypacco-loading-overlay').length === 0) {
        jQuery('body').append(
            '<div id="easypacco-loading-overlay" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.7); z-index: 999999; display: flex; align-items: center; justify-content: center;">' +
...
function hideLoadingOverlay() {
    jQuery('#easypacco-loading-overlay').fadeOut(300, function() {
        jQuery(this).remove();
    });
}
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (!response.success) {
                    ...
                    var trackingUrl = 'https://ordini.easypacco.it/tracking/' + objJSON.id;
                    ...
                    $(orderCard).html(
                        '<div style="padding: 30px; text-align: center;">' +
                            ...
                            '</div>' +
                            '<p style="font-size: 12px; color: #666; margin: 0;">Se hai impostato "Paga con Credito", la spedizione è già attiva. Altrimenti accedi alla tua area privata EasyPacco per completare il pagamento.</p>' +
                        '</div>'
                    );
